...
int main(int argc, char **argv, char **const envp) {
  ...
  if (Loader.Is64BitMode()) {
    if (!Loader.MapMemory([CTX](void *addr, size_t length, int prot, int flags, int fd, off_t offset) {
      ...
      if (rv != MAP_FAILED) {
        FEXCore::Context::SetMemoryMap(CTX, (uintptr_t)rv, length, flags & PROT_WRITE);
      }
      return rv;
    ...
    if (!Loader.MapMemory([&Allocator, CTX](void *addr, size_t length, int prot, int flags, int fd, off_t offset) {
      auto rv = Allocator->mmap(addr, length, prot, flags, fd, offset);
      if (rv != MAP_FAILED) {
        FEXCore::Context::SetMemoryMap(CTX, (uintptr_t)rv, length, flags & PROT_WRITE);
      }
      return rv;
    }, [&Allocator, CTX](void *addr, size_t length) {
